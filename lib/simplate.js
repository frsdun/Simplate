...
(function () {
    ...
    function resolve_display_tokens(html, stored_props, file_name) {
        ...
        props_tokens.forEach(token => {
            ...
            if (!value) {
                console.warn(`Warning in file: "${file_name}". Display token "${token.key}" could not find br found.`);
                value = '';
            }
    ...
    function find_display_tokens(html) {
        ...
        while ((match = pattern.exec(html)) != null) {
            let found_item = {};
            found_item.key = match[0].replace('@display{', '').replace('}@', '');
